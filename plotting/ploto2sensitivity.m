...
xlabel('[NO_2^-] (\muM)')
 ...
 for i = 1:length(o2vals)
    ...
    color = [0, o2/200, (200-o2)/200];
    disp(savestr)
    bgc = load(savestr).bgc;

    subplot(2,2,1)
    plot(bgc.o2, bgc.zgrid, 'color', color)
    hold on; box on;

    subplot(2,2,2)
    plot(bgc.n2o, bgc.zgrid, 'color', color)
    hold on; box on;

    subplot(2,2,3)
    plot(bgc.no3, bgc.zgrid, 'color', color)
    hold on; box on;

    subplot(2,2,4)
    plot(bgc.no2, bgc.zgrid, 'color', color)
    hold on; box on;
end
...
xlabel(insertAfter('\delta^{15}N-N_2O^{\beta} ()','(',char(8240)))
 ...
 for i = 1:length(o2vals)
    ...
    subplot(2,2,1)
    plot(bgc.d15no3, bgc.zgrid, 'color', color)
    hold on; box on;

    subplot(2,2,2)
    plot(bgc.d15no2, bgc.zgrid, 'color', color)
    hold on; box on;

    subplot(2,2,3)
    plot(bgc.d15n2oA, bgc.zgrid, 'color', color)
    hold on; box on;

    subplot(2,2,4)
    plot(bgc.d15n2oB, bgc.zgrid, 'color', color)
    hold on; box on;
end
